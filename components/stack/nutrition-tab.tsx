...
import {
  Apple, Salad, Ban, RefreshCw, ChevronDown, ChevronUp, Loader2,
...
export function NutritionTab() {
  ...
  async function loadPlan(refresh = false) {
    // Cache-or-not behavior must be transparent: only show spinner when no cached plan in state
    if (refresh) setRegenerating(true);
    else if (!plan) setLoading(true);
    setError(null);
    try {
      const r = await fetch(`/api/nutrition/plan${refresh ? "?refresh=1" : ""}`, { cache: "no-store" });
  ...
  const cacheAgeDays = plan?.cached ? daysSince(plan.generatedAt) : null;

  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        {plan?.cached && cacheAgeDays !== null ? (
          <button
            onClick={() => loadPlan(true)}
            disabled={regenerating}
            className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-xs border border-emerald/30 bg-emerald/5 text-emerald hover:bg-emerald/15 transition disabled:opacity-50"
          >
            {regenerating ? <Loader2 className="h-3 w-3 animate-spin" /> : <RefreshCw className="h-3 w-3" />}
            Plan généré il y a {cacheAgeDays === 0 ? "moins d'un jour" : `${cacheAgeDays}j`} — Régénérer
          </button>
        ) : <div />}
          ...
          <nav className="flex gap-1 border-b border-border overflow-x-auto scrollbar-thin" role="tablist">
            {TABS.map((t) => (
              ...
              >
                {t.label}
                {tab === t.key && (
                  <motion.span layoutId="nutrition-subtab" className="absolute inset-x-2 -bottom-px h-0.5 bg-emerald rounded-full" />
                )}
              </button>
            ))}
          </nav>
