...
export async function POST(req: Request) {
  ...
  const r = sqlite.prepare(`INSERT INTO supplement (name, dose, unit, timing, frequency, started_at, ended_at, notes, target_biomarker, target_snp, url, brand, image_url, ingredients, serving_size, suggested_use, price, duration, user_id) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`)
    .run(body.name, body.dose ?? null, body.unit ?? null, body.timing ?? null, body.frequency ?? null, body.startedAt ?? Date.now(), body.endedAt ?? null, body.notes ?? null, body.targetBiomarker ?? null, body.targetSnp ?? null, body.url ?? null, body.brand ?? null, body.imageUrl ?? null, ingredientsJson, body.servingSize ?? null, body.suggestedUse ?? null, body.price ?? null, body.duration ?? null, userId);
